...
void CandidateWindow::onDraw(HDC hDC)
{
    HFONT oldFont;
    RECT rc;
    ...
    for(int i = 0, n = items_.size(); i < n; ++i) {
        ...
        if(col >= candPerRow_) {
            ...
            x += colSpacing_ + selKeyWidth_ + itemsWidth_[i];
        }
    }
...
void CandidateWindow::onPaint(WPARAM wp, LPARAM lp) {
    ...
    //    HBITMAP bmp = CreateCompatibleBitmap(hDC, 2000, 200);   // ��������λͼ����
    //    SelectObject(mdc, bmp);    // ѡ��
    //}
    //onDraw(mdc);

    // 3.һ���Ը��Ƶ��豸DC
    BitBlt(hDC, 0, 0, clientRect_.right - clientRect_.left, clientRect_.bottom - clientRect_.top, mdc, 0, 0, SRCCOPY);
...
void CandidateWindow::recalculateSize() {
    ...
    itemsWidth_.resize(items_.size());
    for(int i = 0, n = items_.size(); i < n; ++i) {
        SIZE selKeySize;
        int lineHeight = 0;
        ...
        itemsWidth_[i] = candidateSize.cx;
...
int  CandidateWindow::getColBeginX(int col) {
    int i;
    int x = 0;
    for (i = 0; i < col; i++)
    {
...
void CandidateWindow::itemRect(int i, RECT& rect) {
    ...
    row = i / candPerRow_;
    col = i % candPerRow_;
    rect.left = margin_ + col * (selKeyWidth_ + colSpacing_) + getColBeginX(col);
    rect.top = margin_ + row * (itemHeight_ + rowSpacing_);
    rect.right = rect.left + (selKeyWidth_ + itemsWidth_[i]);
